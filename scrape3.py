...
try:
    ...
    soup = BeautifulSoup(response.content, "lxml")#.content 作用-原始 bytes，需要你自己決定如何解碼（更安全）
    ...
    if len(book_list_uls) > 2:
        ...
        if isinstance(book_list_ul, Tag):
            ...
            for item in book_list_ul.select(".item"):# 選擇所有 class 為 item 的元素
                ...
                # print("rank_tag:", rank_tag)
                # print("title_tag:", title_tag)

                #print(price_tag)# 內容包含文字跟其他標籤，需要進一步處理

                # 確保所有標籤都存在，若網頁結構改變可能會缺少某些標籤
